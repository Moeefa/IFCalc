          ...
          >
            {data.map((e: any, i: number) => {
              ...
              return (
                <AccordionItem
                  key={i}
                  startContent={<div className={`flex justify-center items-center h-10 w-10 p-5 drop-shadow-lg rounded-full bg-background`}><p>{Number((final + 0.01).toFixed(1)).toLocaleString("pt-BR")}</p></div>}
                  ...
                  aria-label={e.nome}
                  subtitle={<p className={`font-semibold text-transparent bg-clip-text bg-gradient-to-b ${Number((final + 0.01).toFixed(1)) >= 6 ? "from-blue-600 to-blue-800" : isPF ? "from-orange-400 to-orange-600" : e.cursando ? "from-neutral-500 to-neutral-800 dark:from-neutral-200 dark:to-neutral-600" : "from-red-600 to-red-800"}`}>{final >= 6 ? "Aprovado" : isPF ? "Prova Final" : e.cursando ? "Cursando" : "Reprovado"}</p>}
                >
