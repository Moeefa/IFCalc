...
export default function LoginPage({ params }: { params: { suap: string } }) {
  ...
  async function onSubmit(data: LoginFormValues) {
    ...
    const res = await fetch(
      `/api/auth/${params.suap}/login?redirect=${encodeURIComponent(searchParams.get("redirect") || "/")}`,
      {
        method: "POST",
        body: formData,
      },
    ...
    if (res.redirected && res.ok) {
      window.location.href = res.url; // Force full page reload on redirect
    } else {
      const json = await res.json();
      setServerError(json.error || "Erro ao realizar login.");
    }
  }
